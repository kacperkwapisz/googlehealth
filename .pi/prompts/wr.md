...
1. Add or update the relevant package changelog entry under `## [Unreleased]` in `packages/sdk/CHANGELOG.md` and/or `packages/cli/CHANGELOG.md`. Apply the cross-package duplication rule: SDK changes that affect end users get duplicated into the CLI changelog.
...
- Run required checks before committing if code changed (`pnpm lint`, `pnpm build`, `bun test` as appropriate).
