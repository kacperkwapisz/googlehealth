...
5. Check for a changelog entry in the relevant `packages/*/CHANGELOG.md` files (`packages/sdk/CHANGELOG.md`, `packages/cli/CHANGELOG.md`). Report whether an entry exists. If missing, state that a changelog entry is required before merge and that you will add it if the user decides to merge. Verify:
   - Entry uses correct section (`### Breaking Changes`, `### Added`, `### Fixed`, etc.)
   - External contributions include PR link and author: `Fixed foo ([#123](https://github.com/kacperkwapisz/googlehealth/pull/123) by [@user](https://github.com/user))`
   - Breaking changes are in `### Breaking Changes`, not just `### Fixed`
   - SDK changes affecting end users are also reflected in `packages/cli/CHANGELOG.md` (cross-package duplication rule)
6. Check if `packages/sdk/README.md`, `packages/cli/README.md`, or the root `README.md` require modification. This is usually the case when existing features have been changed, or new features have been added.
